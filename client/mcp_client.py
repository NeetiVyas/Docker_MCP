...
from contextlib import AsyncExitStack
from typing import AsyncGenerator, Optional
from schemas.response_schema import parse_tool_response
from dotenv import load_dotenv
from pydantic import BaseModel
...
from schemas.response_schema import BaseToolResponse, ErrorResponse
...
═══════════════════════════════════════════════
CONFIRMATION PROTOCOL — HIGHEST PRIORITY RULE
═══════════════════════════════════════════════
Destructive tools: remove_container, remove_image, stop_container, 
                   remove_network, build_image

When the user requests a destructive action, you MUST follow this EXACT two-turn sequence:

TURN 1 — YOU OUTPUT THIS, NO TOOL CALL:
  "⚠️ I am about to: [describe the action in plain English]
   Are you sure you want to proceed? (yes/no)"
  → STOP. Do not call any tool. End your response here.

TURN 2 — ONLY after the user replies:
  • If user says "yes" → call the tool NOW.
  • If user says "no"  → say "Operation cancelled." and stop.

VIOLATION EXAMPLES (never do these):
  ✗ Calling remove_image and THEN asking for confirmation
  ✗ Asking for confirmation and calling the tool in the same turn
  ✗ Assuming "yes" from context and skipping confirmation
  ✗ Calling the tool, getting a result, and then showing the confirmation prompt

CORRECT EXAMPLE:
  User: "delete image ubuntu"
  You (TURN 1, NO TOOL CALL):
    "⚠️ I am about to remove image 'ubuntu' from your local Docker.
     Are you sure you want to proceed? (yes/no)"
  User: "yes"
  You (TURN 2): [calls remove_image tool]
...
1. Call ONLY ONE tool at a time — never call multiple tools in a single response.
2. Wait for each tool result before deciding what to do next.
3. After receiving a tool result, return the final answer IMMEDIATELY if no more tools are needed.
...
6. If a tool returns status "error", STOP and report the error in plain English.
"""
    ...
    @staticmethod
    def _extract_tool_output(raw) -> tuple[str, BaseToolResponse | None]:
        if hasattr(raw, "content"):
            raw = raw.content
        if isinstance(raw, list):
            parts = []
        ...
        try:
            return text, parse_tool_response(text)
        except Exception:
            return text, None
            ...
            async with asyncio.timeout(AGENT_TIMEOUT_SECONDS):
                async for event in self.agent.astream_events(
                    ...
                    if kind == "on_chat_model_start":
                        ...
                        tool_name  = event["name"]
                        raw_output = event.get("data", {}).get("output", "")
                        text, response_obj = self._extract_tool_output(raw_output)

                        print(f"\nRESULT: {text[:400]}")

                        if response_obj is None:
                            # Could not parse — surface raw text as a plain message
                            yield sse_event("tool_result", {
                                "status": "success",
                                "title":  text,
                                "detail": None,
                            ...
                            tool_error_detected = True
                            # ErrorResponse.display() returns {"status": "error", "message": ...}
                            yield sse_event("tool_result", response_obj.display())
                            _suppress_tokens    = False
                            successful_tool_ran = False
                            full_response       = ""

                        else:
                            # display() returns {status, title, detail, table}
                            yield sse_event("tool_result", response_obj.display())
                            successful_tool_ran = True
                            _suppress_tokens    = True
                            full_response       = ""
